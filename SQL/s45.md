...
The above statement implies that if the maximum number of challenges created is $N$, include all those students who created the maximum number of challenges $N$. The following query returns the maximum number of challenges created:
```
SELECT MAX(C) N
FROM (SELECT COUNT(CHALLENGE_ID) C
      ...
      GROUP BY HACKER_ID);
```
Having computed the maximum number of challenges, $N$, we will include all those students who created $N$ challenges. So, our original query can be improved as follows:
```
SELECT C.HACKER_ID, H.NAME, COUNT(C.CHALLENGE_ID) CNT
FROM HACKERS H, CHALLENGES C
WHERE H.HACKER_ID = C.HACKER_ID 
