...
```
The quantity `COUNT(C.CHALLENGE_ID)` comes from the top-most query block. The `HAVING` clause therefore excludes all those students who happen to have created the same number of challenges.
...
FROM (SELECT COUNT(CHALLENGE_ID) C
      ...
      GROUP BY HACKER_ID);
```
We will now add back all those students who created the maximum number of challenges $N$:
