...
This means, for any two different students, do not include them if the count of the challenges created is equal. We can exclude these rows from our initial query by using a `HAVING ` clause condition as follows:
```
SELECT C.HACKER_ID, H.NAME, COUNT(C.CHALLENGE_ID) CNT
FROM HACKERS H, CHALLENGES C
WHERE H.HACKER_ID = C.HACKER_ID 
...
```
The quantity `COUNT(C.CHALLENGE_ID)` in the `HAVING` clause refers to the count of challenges from the top-most query block. The `HAVING` clause therefore excludes all those students who happen to have created the same number of challenges.
