...
**Step 2**: The following query returns the maximum number of challenges created:
```
SELECT MAX(C) N
...
**Step 3**: We want to exclude those students who created the same number of challenges, if the number of challenges they created is less than $N$.

This means, for any two different students, do not include them if the count of the challenges created is equal. The following query snipet excludes these rows:
```
...
```
Now, putting the above all together, we can construct our desired query as follows:
```
SELECT C.HACKER_ID, H.NAME, COUNT(C.CHALLENGE_ID) CNT
FROM HACKERS H, CHALLENGES C
WHERE H.HACKER_ID = C.HACKER_ID 
