...
### $\text{\sc Min-Heapify}$ Restores the Min-Heap Property

In a min-heap, every element is less than or equal to its children.  During an insert or delete operation in a min-heap, if the min-heap property is violated - that is given an array $A$ and an index $i$ such that the subtrees rooted at `left[i]` and `right[i]` are min-heap, but $A[i]$ is greater than its children - we can use $\text{\sc Min-Heapify}$ to restore the min-heap property.
