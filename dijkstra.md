...
The array representation allows us to access a node's parent and children in $O(1)$ time. If the index of a node is $k$, then the index of its parent is $\lfloor k/2 \rfloor$, and the indices of its right and left children are $2k$ and $2k+1$ respectively.
